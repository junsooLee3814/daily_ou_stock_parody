...
for idx, row in df.iterrows():
    ...
    draw.text((LEFT_MARGIN, y), page_info_text, font=date_font, fill=GRAY_COLOR, spacing=-2)
    y += date_font.size + 20
    ...
    if 'date' in row and pd.notna(row['date']):
        ...
        draw.text((LEFT_MARGIN, y), formatted_date, font=date_font, fill=GRAY_COLOR, spacing=-2)
    ...
    if 'parody_title' in row and pd.notna(row['parody_title']):
        y = draw_text(draw, (LEFT_MARGIN, y), str(row['parody_title']), 
                                        ...
                                        max_text_width, line_spacing_ratio=LINE_SPACING_RATIO, spacing=-2)
        y += SECTION_GAP * 1.5
    ...
    if 'original_title' in row and pd.notna(row['original_title']):
        ...
        draw.text((LEFT_MARGIN, source_y_start), source_text, font=source_font, fill=GRAY_COLOR, spacing=-2)
    ...
    if 'disclaimer' in row and pd.notna(row['disclaimer']):
        ...
        draw_text(draw, (LEFT_MARGIN, disclaimer_y_start), disclaimer_text, 
                        disclaimer_font, GRAY_COLOR, max_text_width, spacing=-2)
