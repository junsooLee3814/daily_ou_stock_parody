...
def fetch_news(rss_url, days=1, min_news=20):
    ...
    for entry in feed.entries:
        ...
        # 날짜 필터링 (엄격하게) - 오늘 날짜만 허용
        if not published_date or published_date != today:
            print(f"[디버그] 날짜 필터링 제외: {published_date} (오늘: {today})")
            filtered_count += 1
            continue
        ...
        print(f"[디버그] 수집: {published_date} - {title[:30]}...")
...
def save_to_gsheet(parody_data_list):
    ...
    try:
        ...
        for i, parody_data in enumerate(parody_data_list, 1):
            ...
            print(f"   - {i}/{len(parody_data_list)} 행 추가 완료")
...
"""
        ...
        if csv_path:
            ...
            print("💡 Google Drive 업로드가 필요하면 올바른 폴더 ID를 설정하세요.")
            # try:
            #     google_drive_folder_id = "YOUR_ACTUAL_FOLDER_ID"  # 실제 Google Drive 폴더 ID로 변경
